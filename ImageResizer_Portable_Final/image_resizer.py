"""
Image Resizer - macOS Compatible Version
Fixed for macOS Tkinter compatibility issues
"""
...
if sys.platform == "darwin":
    # Force Tkinter to use the correct display
    import os
...
class ImageResizerApp:
    ...
    def setup_ui(self):
        """Setup the user interface with macOS compatibility fixes"""
        
        # Force update to ensure GUI renders
        self.root.update_idletasks()
        
        # Title frame with explicit styling
        ...
        # Main container with explicit styling
        main_container = tk.Frame(self.root, bg="white")
        main_container.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Left panel - Controls
        left_panel = tk.Frame(main_container, padx=10, pady=10, bg="white")
        left_panel.pack(side=tk.LEFT, fill=tk.BOTH, expand=False)
        
        # Right panel - Preview with explicit styling
    ...
    def setup_controls(self, parent):
        """Setup the control panel with explicit styling"""
        
        # Upload section
        upload_frame = tk.LabelFrame(parent, text="📁 Upload Image", font=("Arial", 11, "bold"), padx=10, pady=10, bg="white")
        upload_frame.pack(fill=tk.X, pady=(0, 15))
        
        self.file_label = tk.Label(upload_frame, text="No file selected", fg="gray", bg="white")
        ...
        # Settings section
        settings_frame = tk.LabelFrame(parent, text="⚙️ Resize Settings", font=("Arial", 11, "bold"), padx=10, pady=10, bg="white")
        settings_frame.pack(fill=tk.X, pady=(0, 15))
        
        # Dimensions
        dim_frame = tk.Frame(settings_frame, bg="white")
        dim_frame.pack(fill=tk.X, pady=5)
        
        tk.Label(dim_frame, text="Width:", font=("Arial", 10), bg="white").grid(row=0, column=0, sticky=tk.W, padx=5)
        width_entry = tk.Entry(dim_frame, textvariable=self.width_var, width=10, font=("Arial", 10), relief=tk.SUNKEN, borderwidth=2)
        width_entry.grid(row=0, column=1, padx=5)
        width_entry.bind('<KeyRelease>', self.on_width_change)
        
        tk.Label(dim_frame, text="px", font=("Arial", 10), bg="white").grid(row=0, column=2, padx=5)
        
        tk.Label(dim_frame, text="Height:", font=("Arial", 10), bg="white").grid(row=0, column=3, sticky=tk.W, padx=(20, 5))
        height_entry = tk.Entry(dim_frame, textvariable=self.height_var, width=10, font=("Arial", 10), relief=tk.SUNKEN, borderwidth=2)
        height_entry.grid(row=0, column=4, padx=5)
        height_entry.bind('<KeyRelease>', self.on_height_change)
        
        tk.Label(dim_frame, text="px", font=("Arial", 10), bg="white").grid(row=0, column=5, padx=5)
        ...
        # Quality slider
        quality_frame = tk.Frame(settings_frame, bg="white")
        quality_frame.pack(fill=tk.X, pady=10)
        
        self.quality_title_label = tk.Label(quality_frame, text="Quality (compression):", font=("Arial", 10), bg="white")
        self.quality_title_label.pack(anchor=tk.W)
        
        # Quick quality presets
        preset_quality_frame = tk.Frame(quality_frame, bg="white")
        preset_quality_frame.pack(fill=tk.X, pady=5)
        
        tk.Label(preset_quality_frame, text="Quick presets:", font=("Arial", 9), bg="white").pack(side=tk.LEFT, padx=(0, 10))
        ...
        # Slider
        slider_frame = tk.Frame(quality_frame, bg="white")
        ...
        self.quality_label = tk.Label(slider_frame, text="50%", font=("Arial", 10, "bold"), bg="white")
        self.quality_label.pack(side=tk.LEFT, padx=10)
        
        # Output format selection
        format_frame = tk.LabelFrame(parent, text="💾 Output Format", font=("Arial", 11, "bold"), padx=10, pady=10, bg="white")
        format_frame.pack(fill=tk.X, pady=(0, 15))
        
        format_options_frame = tk.Frame(format_frame, bg="white")
        format_options_frame.pack(fill=tk.X, pady=5)
        
        tk.Label(format_options_frame, text="Save as:", font=("Arial", 10), bg="white").pack(side=tk.LEFT, padx=(0, 10))
        ...
        # Web presets
        presets_frame = tk.LabelFrame(parent, text="🌐 Web Presets", font=("Arial", 11, "bold"), padx=10, pady=10, bg="white")
        ...
        # Info section
        self.info_frame = tk.LabelFrame(parent, text="ℹ️ Original Image Info", font=("Arial", 11, "bold"), padx=10, pady=10, bg="white")
        self.info_frame.pack(fill=tk.X, pady=(0, 15))
        
        self.info_label = tk.Label(self.info_frame, text="Upload an image to see details", fg="gray", justify=tk.LEFT, bg="white")
    ...
    def setup_preview_panel(self, parent):
        """Setup the preview panel with explicit styling"""
        ...
        self.preview_canvas = tk.Canvas(canvas_frame, bg="white", highlightthickness=1, highlightbackground="#ccc")
    ...
    # (upload_image, update_preview, save_image, etc.)
    ...
    def update_preview(self):
        ...
        try:
            ...
            if self.crop_mode.get() and self.needs_crop and self.crop_preview_image:
                ...
                tk.Label(
                    preview_frame,
                    text="📍 Crop Area (orange border shows what will be kept)",
                    font=("Arial", 10, "bold"),
                    bg="white"
                ).pack(pady=5)
                ...
                crop_label = tk.Label(preview_frame, image=crop_photo, bg="white")
                crop_label.image = crop_photo
                crop_label.pack(pady=5)
                
                tk.Label(preview_frame, text="↓", font=("Arial", 16), bg="white").pack(pady=5)
                ...
                result_label = tk.Label(preview_frame, image=result_photo, bg="white")
