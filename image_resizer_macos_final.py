...
class ImageResizerApp:
    ...
    def create_crop_preview_image(self):
        """Create a preview image showing the crop area"""
        if not self.original_image:
            return None
        ...
        try:
            ...
            # Add text
            draw.text((crop_box[0] + 5, crop_box[1] + 5), "Crop Area", fill="red")
    ...
    def open_interactive_crop(self):
        ...
        self.crop_interactive_window.title("Interactive Crop Positioning")
        self.crop_interactive_window.geometry("800x600")
        
        # Create canvas for crop preview
        crop_canvas = tk.Canvas(self.crop_interactive_window, bg="white")
        crop_canvas.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Convert PIL image to PhotoImage
        crop_photo = ImageTk.PhotoImage(crop_preview)
        crop_canvas.create_image(0, 0, anchor=tk.NW, image=crop_photo)
        crop_canvas.image = crop_photo  # Keep a reference
        
        # Bind mouse events for dragging
        crop_canvas.bind("<Button-1>", lambda e: self.on_crop_drag_start(e, crop_canvas))
        crop_canvas.bind("<B1-Motion>", lambda e: self.on_crop_drag_motion(e, crop_canvas))
        crop_canvas.bind("<ButtonRelease-1>", lambda e: self.on_crop_drag_end(e, crop_canvas))
        ...
        instructions = tk.Label(
            ...
            text="Drag the red rectangle to adjust crop position. Click 'Apply & Close' when done.",
            font=("Arial", 10),
            fg="blue"
        )
        instructions.pack(pady=5)
    
    def on_crop_drag_start(self, event, canvas):
        """Handle start of crop drag"""
        self.dragging = True
        self.drag_start_x = event.x
        self.drag_start_y = event.y
    
    def on_crop_drag_motion(self, event, canvas):
        ...
        # Update preview
        self.update_interactive_crop_display(canvas)
        
        # Update drag start position
        self.drag_start_x = event.x
        self.drag_start_y = event.y
    
    def on_crop_drag_end(self, event, canvas):
        """Handle end of crop drag"""
        self.dragging = False
    
    def update_interactive_crop_display(self, canvas):
        """Update the interactive crop display"""
        if not self.original_image:
            return
        
        # Create new crop preview
        crop_preview = self.create_crop_preview_image()
        if crop_preview:
            # Convert to PhotoImage
            crop_photo = ImageTk.PhotoImage(crop_preview)
            canvas.delete("all")
            canvas.create_image(0, 0, anchor=tk.NW, image=crop_photo)
            canvas.image = crop_photo  # Keep a reference
    ...
    def reset_crop_position(self):
        ...
        if self.crop_interactive_window:
            # Find the canvas in the crop window
            for widget in self.crop_interactive_window.winfo_children():
                if isinstance(widget, tk.Canvas):
                    self.update_interactive_crop_display(widget)
                    break
...
def main():
    print("Starting Image Resizer - Full Featured macOS Version...")
    ...
    try:
        ...
        print("This version includes all features from the Windows version")
